...
const classificationCache = new Map<
  string,
  { result: boolean; timestamp: number; type: string }
>();
...
export async function classifyImage(
  ...
  try {
    ...
    console.log(
      "Classification error - allowing image to proceed with analysis",
    );
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (roofInput?.roofSize) {
        const roofSize = roofInput.roofSize;
        if (roofSize < 800) {
          panelSizeAdjustment =
      ...
      if (!responseText) {
        throw new Error("No response from AI installation analysis");
      }
      ...
      // Use calculation-service for accurate efficiency scoring (multiplicative model)
      const efficiencyScore = calculateEfficiencyScore(
        aiResult.primaryOrientation,
        aiResult.pitchCategory,
        aiResult.shadingLevel,
      );
      ...
      // Calculate final metrics using calculation-service
      const finalPowerOutputKW = calculatePowerOutput(actualPanelsPlaced);
      const coveragePercentage =
        Math.round(
          ((actualPanelsPlaced * MARKET_STANDARDS.panelArea) / finalRoofArea) *
            100 *
            100,
        ) / 100;
      ...
      const installationCost = calculateInstallationCost(actualPanelsPlaced);
      ...
      console.log(
        `Backend Calculations: ${actualPanelsPlaced} panels placed on roof, ${coveragePercentage}% coverage, ${finalPowerOutputKW}kW power, ${efficiencyScore}% efficiency`,
      );
      console.log(
        `Financial: ₹${annualSavings} annual savings, ₹${installationCost} cost, ${paybackPeriod}yr payback`,
      );

      // Build result with calculation details for PDF and UI
      const result_data: InstallationResult = {
        totalPanels: actualPanelsPlaced,
        ...
        calculationDetails: {
          marketStandards: {
            panelWidth: MARKET_STANDARDS.panelWidth,
            panelHeight: MARKET_STANDARDS.panelHeight,
            panelArea: MARKET_STANDARDS.panelArea,
            panelPower: MARKET_STANDARDS.panelPower,
            panelPowerKW: MARKET_STANDARDS.panelPowerKW,
          },
          annualSavings,
          installationCost,
          paybackPeriod,
...
export async function analyzeFaultsWithAI(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (!responseText) {
        throw new Error("No response from AI fault detection");
      }
