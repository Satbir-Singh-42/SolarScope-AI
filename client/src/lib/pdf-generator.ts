...
// RGB color tuple type for jsPDF methods
...
function addProfessionalHeader(pdf: jsPDF, title: string, subtitle: string) {
...
function addMetricCard(
  ...
  if (value.includes("₹")) {
    ...
    pdf.setFontSize(14);
    pdf.text(rupeeValue, x + width / 2 + 6, y + height / 2 + 2, {
      align: "center",
    });
  } else {
...
function checkPageSpace(
  ...
  if (currentY + neededSpace > pageHeight - 30) {
    ...
    pdf.text(`Page ${pdf.getNumberOfPages()}`, pageWidth - 20, 15, {
      align: "right",
    });
...
export async function generateInstallationPDF(
  ...
  try {
    ...
    pdf.text(
      `Report ID: INS-${Date.now().toString().slice(-8)}`,
      pageWidth - 20,
      yPosition,
      { align: "right" },
    );
    ...
    yPosition = addSectionHeader(pdf, "SYSTEM OVERVIEW", yPosition);
    ...
    yPosition = addKeyValuePair(
      ...
      yPosition,
      "Roof Type",
      result.roofType || "Standard Residential",
    );
    yPosition = addKeyValuePair(
      pdf,
      25,
      yPosition,
      "Usable Area",
      `${result.usableRoofArea || "Auto-calculated"} sq ft`,
    );
    yPosition = addKeyValuePair(
      pdf,
      25,
      yPosition,
      "System Orientation",
      result.orientation || "Optimized for Maximum Solar Gain",
    );
    ...
    yPosition = addSectionHeader(pdf, "VISUAL ANALYSIS", yPosition);
    ...
    yPosition = addSectionHeader(pdf, "PERFORMANCE PROJECTIONS", yPosition);

    // Use calculation details from backend when available, otherwise compute locally
    const actualPanelWattage =
      result.calculationDetails?.marketStandards?.panelPower || 400;
    ...
    const estimatedSavings =
      result.calculationDetails?.annualSavings ??
      Math.round(annualProduction * 6.5);
    ...
    addMetricCard(
      ...
      perfCardHeight,
      "Monthly Production",
      monthlyProduction.toString(),
      "kWh",
      colors.primary,
    );
    addMetricCard(
      pdf,
      perfStartX + perfCardWidth + perfSpacing,
      yPosition,
      perfCardWidth,
      perfCardHeight,
      "Annual Production",
      annualProduction.toString(),
      "kWh/year",
      colors.secondary,
    );
    // Format annual savings with proper Indian number formatting
    const formattedSavings = new Intl.NumberFormat("en-IN").format(
      estimatedSavings,
    );
    addMetricCard(
      pdf,
      perfStartX + 2 * (perfCardWidth + perfSpacing),
      yPosition,
      perfCardWidth,
      perfCardHeight,
      "Annual Savings",
    ...
    if (result.notes && result.notes.trim()) {
      ...
      yPosition = addSectionHeader(pdf, "INSTALLATION NOTES", yPosition);
      ...
      const cleanedNotes = result.notes
        ...
        .split("\n")
        .filter(
          (line) => line.trim() !== "" && !line.includes("Installation Notes:"),
        )
        .map((line) => line.trim());
    ...
    yPosition = checkPageSpace(pdf, yPosition, 100);
    yPosition = addSectionHeader(pdf, "TECHNICAL SPECIFICATIONS", yPosition);
    ...
    yPosition = addKeyValuePair(
      ...
      yPosition,
      "Panel Type",
      "Monocrystalline Silicon (400W each)",
    );
    yPosition = addKeyValuePair(
      pdf,
      25,
      yPosition,
      "System Efficiency",
      `${result.efficiency}% module efficiency`,
    );
    yPosition = addKeyValuePair(
      pdf,
      25,
      yPosition,
      "Annual Production",
      `${annualProduction.toLocaleString("en-IN")} kWh/year`,
    );
    yPosition = addKeyValuePair(
      pdf,
      25,
      yPosition,
      "System Lifespan",
      "25+ years with manufacturer warranty",
    );
    yPosition = addKeyValuePair(
      pdf,
      25,
      yPosition,
      "Payback Period",
      result.calculationDetails?.paybackPeriod
        ? `${result.calculationDetails.paybackPeriod} years`
        : "6-8 years (estimated)",
    );
    yPosition = addKeyValuePair(
      pdf,
      ...
      yPosition,
      "Installation Cost",
      result.calculationDetails?.installationCost
        ? `₹${new Intl.NumberFormat("en-IN").format(result.calculationDetails.installationCost)}`
    ...
    pdf.setTextColor(...colors.textMuted);
    pdf.text(
      "Generated by SolarScope AI - " + new Date().toLocaleDateString(),
      pageWidth / 2,
      footerY,
      { align: "center" },
    );
...
export async function generateFaultDetectionPDF(
  ...
  try {
    ...
    pdf.text(`Report Generated: ${reportDate}`, 20, yPosition);
    pdf.text(
      `Report ID: FLT-${Date.now().toString().slice(-8)}`,
      pageWidth - 20,
      yPosition,
      { align: "right" },
    );
    ...
    const faultCounts = result.faults.reduce(
      (acc, fault) => {
    ...
    const criticalColor =
      faultCounts.Critical > 0 ? colors.danger : colors.textLight;
    ...
    addMetricCard(
      ...
      cardHeight,
      "Critical Severity",
      (faultCounts.Critical || 0).toString(),
      "urgent",
      criticalColor,
    );
    ...
    yPosition = addSectionHeader(pdf, "SYSTEM HEALTH OVERVIEW", yPosition);
    ...
    yPosition = addKeyValuePair(
      ...
      yPosition,
      "Medium Priority",
      `${faultCounts.Medium || 0} maintenance items`,
    );
    yPosition = addKeyValuePair(
      pdf,
      25,
      yPosition,
      "Low Priority",
      `${faultCounts.Low || 0} minor issues`,
    );
    yPosition = addKeyValuePair(
      pdf,
      25,
    ...
    if (result.faults.length > 0) {
      yPosition = checkPageSpace(pdf, yPosition, 60);
      yPosition = addSectionHeader(pdf, "DETAILED FAULT ANALYSIS", yPosition);
      ...
      result.faults.forEach((fault, index) => {
        ...
        let severityColor: RGB = colors.textLight;
        let severityBgColor: RGB = [240, 240, 240];
        ...
        pdf.setFontSize(10);
        pdf.setFont("helvetica", "bold");
        pdf.text((index + 1).toString(), 25, yPosition + 4.5, {
          align: "center",
        });
        ...
        pdf.text(locationText, pageWidth - 60, yPosition + 4, {
          align: "right",
        });
    ...
    const criticalFaults = result.faults.filter(
      (f) => f.severity.toLowerCase() === "critical",
    ).length;
    const highFaults = result.faults.filter(
      (f) => f.severity.toLowerCase() === "high",
    ).length;

    if (criticalFaults > 0 || highFaults > 0) {
      yPosition = checkPageSpace(pdf, yPosition, 60);
      yPosition = addSectionHeader(pdf, "PRIORITY ACTIONS REQUIRED", yPosition);
      ...
      if (criticalFaults > 0) {
        yPosition = addKeyValuePair(
          ...
          yPosition,
          "IMMEDIATE ACTION",
          `${criticalFaults} critical issue(s) require immediate attention`,
          colors.danger,
          colors.danger,
        );
      }
      if (highFaults > 0) {
        yPosition = addKeyValuePair(
          ...
          yPosition,
          "URGENT REPAIR",
          `${highFaults} high-priority issue(s) need prompt repair`,
          colors.warning,
          colors.warning,
        );
      }
