...
import { AlertTriangle, CheckCircle, Eye, FileDown, X } from "lucide-react";
...
import { compressImage, cleanupOldImages } from "@/lib/image-utils";
...
export default function FaultDetection() {
  ...
  useEffect(() => {
    if (analysisResults.length > 0) {
      try {
        ...
        console.error(
          "Error saving fault detection results to storage:",
          error,
        );
        ...
        try {
          sessionStorage.setItem(
            "faultDetectionResults",
            JSON.stringify(analysisResults),
          );
        } catch (retryError) {
          console.warn(
            "Failed to save fault detection results even after cleanup:",
            retryError,
          );
        }
      }
  ...
  const analysisMutation = useMutation({
    ...
    onSuccess: async (data, variables) => {
      ...
      toast({
        ...
        description:
          "Your solar panel analysis has been completed successfully.",
        variant: "success",
      });
    },
    onError: (error) => {
      ...
      if (error instanceof Error) {
        ...
        if (
          errorText.includes("not solar panel") ||
          errorText.includes("no solar") ||
          errorText.includes("rooftop")
        ) {
          errorTitle = "Invalid Image for Fault Detection";
          errorMessage =
            "Please upload a solar panel image. This feature analyzes existing solar panel installations for defects and performance issues, not rooftops.";
        } else if (
          errorText.includes("indoor") ||
          errorText.includes("interior")
        ) {
          errorTitle = "Indoor Image Detected";
          errorMessage =
            "Please upload an outdoor solar panel image. Indoor or interior photos cannot be used for fault detection analysis.";
        } else if (
          ...
          errorMessage =
            "Please upload a clear image showing solar panels or photovoltaic equipment for fault detection analysis.";
        } else if (
          ...
          errorMessage =
            "The AI analysis service is currently overloaded. Please wait a few minutes and try again. If the issue persists, try a different image or contact support.";
          ...
          errorText.includes("backend not connected") ||
          errorText.includes("server not available")
        ) {
          errorTitle = "Backend Connection Error";
          errorMessage =
            "The backend server is not connected properly. Please check the server status and try again.";
        } else if (
  ...
  const clearAllResults = () => {
    ...
    toast({
      ...
      description:
        "All fault detection results and uploaded images have been cleared.",
      variant: "success",
    });
  };
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      toast({
        ...
        description:
          "First fault detection report has been downloaded as a PDF.",
        variant: "success",
      });
    } catch (error) {
      ...
      toast({
        title: "Export Failed",
        description:
          "Failed to generate individual PDF report. Please try again.",
        variant: "destructive",
      });
    }
  };
  ...
  const exportSummaryPDF = async () => {
    ...
    try {
      ...
      const summaryResult = {
        ...
        overallHealth: analysisResults.some((r) =>
          r.faults.some((f) => f.severity === "Critical"),
        )
          ? "Critical Issues Found"
          : analysisResults.some((r) =>
                r.faults.some((f) => f.severity === "High"),
              )
            ? "High Risk Issues"
            : analysisResults.some((r) => r.faults.length > 0)
              ...
              : "Good Condition",
        recommendations: [
          ...new Set(analysisResults.flatMap((r) => r.recommendations)),
        ],
      };
      ...
      toast({
        ...
        description:
          "Complete fault detection summary with all issues has been downloaded as a PDF.",
        variant: "success",
      });
    } catch (error) {
      console.error("PDF generation failed:", error);
      toast({
        title: "Export Failed",
        description:
          "Failed to generate collective summary PDF. Please try again.",
        variant: "destructive",
      });
    }
  };
  ...
  const exportShortSummaryPDF = async () => {
    ...
    try {
      ...
      const criticalFaults = analysisResults.flatMap((r) =>
        r.faults.filter((f) => f.severity === "Critical"),
      );
      const highFaults = analysisResults.flatMap((r) =>
        r.faults.filter((f) => f.severity === "High"),
      );
      const totalFaults = analysisResults.reduce(
        (sum, r) => sum + r.faults.length,
        0,
      );
      ...
      toast({
        title: "Export Failed",
        description:
          "Failed to generate executive summary PDF. Please try again.",
        variant: "destructive",
      });
    }
  };
  ...
  const getIssuesByType = (type: string) => {
    return analysisResults.reduce((total, result) => {
      return (
        total +
        result.faults.filter((fault) => fault.severity.toLowerCase() === type)
          .length
      );
    }, 0);
  };
                  ...
                  <div className="space-y-2 sm:space-y-3">
                    <div className="bg-gray-50 rounded-lg p-2 sm:p-3 lg:p-4">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-1 sm:mb-2 gap-1 sm:gap-2">
                        <h5 className="font-semibold text-sm sm:text-base lg:text-lg text-primary-custom">
                    ...
                    {result.faults.map((fault, faultIndex) => (
                      <div
                        key={faultIndex}
                        className={`p-2 sm:p-3 lg:p-4 rounded-lg border-l-4 ${
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-1 sm:mb-1.5 gap-1">
                          ...
                          <span
                            className={`text-xs px-1.5 sm:px-2 py-0.5 sm:py-1 rounded self-start ${
                              fault.severity === "Critical"
                    ...
                    {result.faults.length > 0 && (
                      <div className="mt-3 p-3 bg-gray-50 rounded-lg border">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-sm">
                          <div
                            className={`bg-white rounded p-2 border-l-4 ${
                              result.faults.some(
                    ...
                    {result.faults.length === 0 && (
                      <div className="p-3 sm:p-4 lg:p-6 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg border border-green-200">
                        <div className="flex items-center space-x-2 sm:space-x-3 mb-2 sm:mb-3">
                          ...
                          <div>
                            <h6 className="font-semibold text-green-800 text-xs sm:text-sm lg:text-base">
                              No Issues Detected
                            </h6>
                            <p className="text-xs text-green-600">
                              Panel is operating in excellent condition
                            </p>
                          </div>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-sm">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        detailsModal.className =
                          "fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[10005] p-4";
                ...
                <CardContent className="p-3 sm:p-4 lg:p-6">
                  <div className="bg-gray-100 rounded-lg flex items-center justify-center h-32 sm:h-40 lg:h-48 mb-3 sm:mb-4">
                    <div className="text-center px-3">
                      ...
                      <p className="text-secondary-custom text-xs sm:text-sm mb-3 sm:mb-4">
                        Analyzing solar panel...
                      </p>
                      ...
                      <p className="text-xs text-gray-500">
                        AI fault detection in progress
                      </p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    {[...Array(2)].map((_, i) => (
                      <div
                        key={i}
                        className="bg-gray-100 animate-pulse rounded h-2 sm:h-3 lg:h-4"></div>
                    ))}
                  </div>
                </CardContent>
              </Card>
          ...
          {analysisResults.length > 0 && (
            <Card className="shadow-material">
              <CardContent className="p-3 sm:p-4 lg:p-6 xl:p-8">
                <h3 className="text-lg sm:text-xl lg:text-2xl font-bold mb-3 sm:mb-4 lg:mb-6 text-primary-custom">
                  Diagnostic Summary
                </h3>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6 mb-4 sm:mb-6 lg:mb-8">
                  ...
                  <div className="text-center">
                    <div className="text-xl sm:text-2xl lg:text-3xl font-bold text-yellow-600 mb-1 sm:mb-2">
                      {getIssuesByType("medium") + getIssuesByType("low")}
                    </div>
                    <div className="text-xs sm:text-sm text-secondary-custom">
                      Medium & Low Issues
                    </div>
                  ...
                  <div className="text-center">
                    <div className="text-xl sm:text-2xl lg:text-3xl font-bold text-green-500 mb-1 sm:mb-2">
                      {
                        analysisResults.filter((r) => r.faults.length === 0)
                          .length
                      }
                    </div>
                    <div className="text-xs sm:text-sm text-secondary-custom">
                ...
                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-3 sm:p-4 lg:p-6 mb-3 sm:mb-4 lg:mb-6 border border-blue-200">
                  <div className="flex items-center space-x-2 mb-3 sm:mb-4">
                    ...
                    <h4 className="text-sm sm:text-base lg:text-lg font-semibold text-blue-900">
                  ...
                  <div className="space-y-4">
                    ...
                    {(() => {
                      ...
                      const uniqueFaultTypes = [
                        ...new Set(
                          analysisResults.flatMap((r) =>
                            r.faults.map((f) => f.type),
                          ),
                        ),
                      ];
                      ...
                      return uniqueFaultTypes.map((faultType) => {
                        ...
                        const relevantRecommendations = analysisResults
                          .filter((r) =>
                            r.faults.some((f) => f.type === faultType),
                          )
                          .flatMap((r) => r.recommendations)
                          .filter(
                            (rec, index, self) => self.indexOf(rec) === index,
                          ) // Remove duplicates
                        ...
                        const colors =
                          severityColors[
                            highestSeverity as keyof typeof severityColors
                          ];
                          ...
                          <div
                            ...
                            <div className="flex items-center space-x-2 mb-2">
                              ...
                              <h5
                                ...
                                {faultType} Issues ({faultsOfType.length}{" "}
                                detected - {highestSeverity} Priority)
                              </h5>
                            </div>
                            ...
                            <div className="space-y-2">
                              <div
                                className={`text-xs sm:text-sm ${colors.text} font-medium mb-1`}>
                                Maintenance Plan:
                              </div>
                              <ul
                                className={`space-y-1 text-xs sm:text-sm ${colors.text.replace("800", "700")}`}>
                                {relevantRecommendations.length > 0 ? (
                                  relevantRecommendations.map((rec, index) => (
                                    <li
                                      ...
                                      <span
                                        className={`${colors.text.replace("800", "500")} font-bold mt-0.5`}>
                                        •
                                      </span>
                                  ...
                                  <li className="flex items-start space-x-2">
                                    ...
                                    <span className="flex-1">
                                      AI analysis in progress - specific
                                      recommendations will be generated based on
                                      fault severity and type
                                    </span>
                                  </li>
                                )}
                              </ul>
                      ...
                      <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                        ...
                        <ul className="space-y-1 text-xs sm:text-sm text-blue-700">
                          <li className="flex items-start space-x-2">
                            ...
                            <span className="flex-1">
                              Schedule professional inspection every 6 months
                          ...
                          <li className="flex items-start space-x-2">
                            ...
                            <span className="flex-1">
                              Document all maintenance activities and track
                              performance metrics
                            </span>
                          </li>
                          <li className="flex items-start space-x-2">
                            ...
                            <span className="flex-1">
                              Contact certified solar technicians for all
                              electrical work and critical repairs
                            </span>
                          </li>
                        </ul>
                      </div>
