...
import { Bot, Send, X, Mic, MicOff, Copy, MessageCircle } from "lucide-react";
    ...
    return [
      {
        id: 1,
        message:
          "Hello! I'm your AI assistant ready to help with solar panel questions. Ask about installation planning, fault detection, or maintenance tips.",
        sender: "ai",
        timestamp: new Date(),
      },
    ];
      ...
      setMessages([
        {
          id: 1,
          message:
            "Hello! I'm your AI assistant. How can I help you with solar panel analysis today?",
          sender: "ai",
          timestamp: new Date(),
        },
      ]);
  ...
  const startTypingAnimation = (messageId: number, fullText: string) => {
    ...
    const typeNextCharacter = () => {
      if (index < fullText.length) {
        ...
        index++;
        typingTimeoutRef.current = setTimeout(typeNextCharacter, 4); // Much faster typing
      } else {
    ...
    try {
      ...
      if (data.error) {
        if (
          ...
          aiResponseText =
            "I'm currently experiencing high demand. Please wait a moment and try again. The AI service is temporarily overloaded.";
        } else {
    ...
    <div className="fixed bottom-4 right-4 z-[9999]">
      ...
      {isOpen && (
        <div className="absolute bottom-16 right-0 mb-2" ref={widgetRef}>
          <Card
            className={`w-[calc(100vw-2rem)] max-w-sm md:max-w-md lg:max-w-lg transition-all duration-300 h-[70vh] max-h-96 md:max-h-[500px] shadow-2xl drop-shadow-lg rounded-lg`}>
            ...
            <CardContent className="p-0 flex flex-col h-[calc(100%-3rem)]">
              ...
              <ScrollArea
                ref={scrollRef}
                className="flex-1 p-3 sm:p-4 pb-0"
                        ...
                        {msg.sender === "ai" && typingMessageId !== msg.id && (
                          <div className="flex justify-end mt-2">
                            <Button
                              ...
                              size="sm"
                              onClick={() =>
                                copyToClipboard(msg.message, msg.id)
                              }
                              ...
                              <Copy
                                className={`h-3 w-3 ${copiedMessageId === msg.id ? "text-green-600" : "text-gray-600"}`}
                              />
                            </Button>
                          </div>
                        )}
              ...
              <div className="p-3 sm:p-4 border-t bg-gray-50 rounded-b-lg">
                <div className="flex items-center gap-3 sm:gap-4">
                  <Input
                    type="text"
                    placeholder={
                      isListening ? "Listening..." : "Ask about solar panels..."
                    }
                    value={inputValue}
